...
class Carrier {
    ...
    constructor( carrierPosX, carrierPosY, carrierWidth, carrierHeight, tilesCount, _tilesOffscreen, _tilePadding, ... ) {
        ...
        if( fileSelector != null ) selector = fe.add_image( fileSelector, 0, 0, tileWidth,tileHeight)
        else {
...
animation.add( PropertyAnimation( selector, 
				{  when = Transition.ToNewList,
				   property = "position",
					start = { x = tilesTotal / 2 * tileWidth + surfacePos - surfacePosOffset, y =470 },
					end ={ x =tilesTotal / 2 * tileWidth + surfacePos - surfacePosOffset, y = 460 },
					loop= true,					
					time = 600
				} ) );	
